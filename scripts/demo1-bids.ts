...
async function main() {
  ...
  await auction.connect(s1).bid(1, { value: ethers.utils.parseEther(`${s1Bid.toFixed(2)}`) });
  highestBid = s1Bid;
  s2Bid = highestBid + step - s2Bid;
  await auction.connect(s2).bid(1, { value: ethers.utils.parseEther(`${s2Bid.toFixed(2)}`) });
  highestBid = highestBid + s2Bid;
  s3Bid = highestBid + step - s3Bid;
  await auction.connect(s3).bid(1, { value: ethers.utils.parseEther(`${s3Bid.toFixed(2)}`) });
