...
async function main() {
  ...
  console.log(s4Bid);
  await auction.connect(s4).bid(2, { value: ethers.utils.parseEther(`${s4Bid.toFixed(2)}`) });
  highestBid = s4Bid;
  s5Bid = highestBid + step - s5Bid;
  console.log(s5Bid);
  await auction.connect(s5).bid(2, { value: ethers.utils.parseEther(`${s5Bid.toFixed(2)}`) });
  highestBid = highestBid + s5Bid;
  s6Bid = highestBid + step - s6Bid;
  console.log(s6Bid);
  await auction.connect(s6).bid(2, { value: ethers.utils.parseEther(`${s6Bid.toFixed(2)}`) });
